...
function getApiUrl(): string {
	const value = import.meta.env?.WORDPRESS_API_URL ?? process.env.WORDPRESS_API_URL;
	if (!value) {
...
/** 投稿タイプ learn を取得（learn_cat を categories として扱い、URL を /learn/{cat}/{slug}/ に正規化） */
export function getLearnPosts(): Promise<WpPost[]> {
	learnPostCache ??= (async () => {
		const [posts, terms] = await Promise.all([
			fetchCollection<WpLearnPostResponse>(LEARN_POST_TYPE, {
		...
		return posts.map((post) => {
			const categories = post.learn_cat ?? [];
			const category = pickDeepestCategory(categories, terms);
			const articleSlug = post.slug.replace(/\.html$/i, "");
			const link = category ? `/learn/${category.slug}/${articleSlug}/` : `/learn/${articleSlug}/`;
			return {
				...post,
				categories,
				link,
...
/** 所属タームのうち、階層が最も深いものを選ぶ */
function pickDeepestCategory(categoryIds: number[], terms: WpCategory[]): WpCategory | undefined {
	const assigned = terms.filter((term) => categoryIds.includes(term.id));
	if (assigned.length === 0) return undefined;

	function depth(term: WpCategory): number {
		let value = 0;
		let current: WpCategory | undefined = term;
		const seen = new Set<number>();
		while (current?.parent) {
			if (seen.has(current.id)) break;
			seen.add(current.id);
			const parent = terms.find((item) => item.id === current!.parent);
			if (!parent) break;
			value += 1;
			current = parent;
		}
...
export type LearnArticleRedirect = {
	/** 旧 URL の記事スラッグ（/learn/{fromSlug}/） */
	fromSlug: string;
	/** 新 URL（/learn/{category}/{slug}/） */
	to: string;
};

/**
 * 旧 /learn/{slug}/ → 新 /learn/{category}/{slug}/ のリダイレクト一覧
 * カテゴリ一覧とスラッグが衝突する場合は除外（カテゴリページを優先）
 */
export async function getLearnArticleRedirects(): Promise<LearnArticleRedirect[]> {
	const [posts, terms] = await Promise.all([getLearnPosts(), getLearnTerms()]);
	const categorySlugs = new Set(terms.map((term) => term.slug));
	const redirects: LearnArticleRedirect[] = [];

	for (const post of posts) {
		const articleSlug = post.slug.replace(/\.html$/i, "");
		const to = toArticlePath(post.link);
		const segments = to.split("/").filter(Boolean);
		if (segments.length < 3) continue;
		if (categorySlugs.has(articleSlug)) continue;
		if (to === `/learn/${articleSlug}/`) continue;
		redirects.push({ fromSlug: articleSlug, to });
	}

	return redirects;
}

/** 本番 nginx 用の location スニペット */
export function formatLearnRedirectsNginx(redirects: LearnArticleRedirect[]): string {
	const lines = [
		"# Generated by Astro build (learn article slug redirects).",
		"# Include inside the server { } block:",
		"#   include /path/to/learn-article-redirects.nginx;",
		"",
	];
	for (const { fromSlug, to } of redirects) {
		lines.push(`location = /learn/${fromSlug} { return 301 ${to}; }`);
		lines.push(`location = /learn/${fromSlug}/ { return 301 ${to}; }`);
	}
	return `${lines.join("\n")}\n`;
}
 ...
 * 例: /learn/html5-guidelines/intro/ → { section: "learn", slug: "html5-guidelines/intro" }
 */
	...
	try {
		...
		return data
			.filter((item) => {
				...
				const link = cachedPost?.link ?? item.link;
				return Boolean(link && isArchiveArticleLink(link));
			})
